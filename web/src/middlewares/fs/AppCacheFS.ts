...
const {_log, _err} = devConsole('[AppCacheFs]');
...
export class AppCacheFs implements Fs {
  ...
  public static async open(
    cacheName: string = VPUBFS_CACHE_NAME,
  ): Promise<AppCacheFs> {
    // キャッシュを開く
  ...
  public async writeFile(
    ...
    try {
      ...
      _log('appCacheFs.writeFile', filePath, contentType);
      const headers = new Headers();
      headers.append('content-type', contentType);
      _log('headers', file, contentType);
      await this.cache.delete(filePath);
      await this.cache.put(filePath, new Response(data, {headers}));
      _log('appCacheFs.writeFile complete');
    } catch (error) {
      _err(error);
    }
  }
