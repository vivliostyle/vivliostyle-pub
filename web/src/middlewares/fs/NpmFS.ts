...
const {_log, _err} = devConsole('[NpmFs]');
...
export class NpmFs implements Fs {
  ...
  private constructor(
    ...
    _log('constructor:', owner, repo, root);
    this.owner = owner;
    this.repo = repo;
    this.branch = branch;
    this.root = root;
  ...
  public async readFile(
    ...
    if (token) {
      _log('use Personal access token');
      octokit = new Octokit({auth: token});
    } else {
      // 1時間あたりのアクセス数制限あり
      _log('not use Personal access token');
      octokit = new Octokit();
    }
    // TODO: Monorepoではない公式テーマはどうする?
    const repoPath = `packages/${this.root}/${path}`;
    _log('repoPath', repoPath);
